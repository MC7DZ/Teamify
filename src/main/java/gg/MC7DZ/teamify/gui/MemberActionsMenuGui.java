    ...
    protected void build() {
        ...
        if (itemsCfg != null) {
            ...
            if (itemsCfg.contains("kick")) {
                ...
                placeConfigItem(inv, kickSlot, itemsCfg.getConfigurationSection("kick"),
                        "player", targetName, "role", targetRole.name());
            }
            if (itemsCfg.contains("promote")) {
                promoteSlot = itemsCfg.getInt("promote.slot", -1);
                placeConfigItem(inv, promoteSlot, itemsCfg.getConfigurationSection("promote"),
                        "player", targetName, "role", targetRole.name());
            }
            if (itemsCfg.contains("demote")) {
                demoteSlot = itemsCfg.getInt("demote.slot", -1);
                placeConfigItem(inv, demoteSlot, itemsCfg.getConfigurationSection("demote"),
                        "player", targetName, "role", targetRole.name());
            }
            if (itemsCfg.contains("transfer")) {
                transferSlot = itemsCfg.getInt("transfer.slot", -1);
                placeConfigItem(inv, transferSlot, itemsCfg.getConfigurationSection("transfer"),
                        "player", targetName, "role", targetRole.name());
            }
        }
